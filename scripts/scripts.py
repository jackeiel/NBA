...
def todays_bets(book='Caesars', display=True, save=True):
    ...
    def internal_predict(row): # must be abbrevs
        sleep(10)
    ...
    #TODO write bet algo
    def place_bet(row):
        if max(row.vegas_line_home, row.predicted_spread) - min(row.vegas_line_home, row.predicted_spread) > 4:
    ...
    full['bet'] = full.apply(place_bet, axis=1)
